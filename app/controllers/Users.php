  ...
  public function register() {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      if (empty($data['password'])) {
        ...
        $data['password_err'] = 'password cannot be less than 3 characters';
      }
      if (empty($data['confirm_password'])) {
        $data['confirm_password_err'] = 'you need to confirm the password';
      } else if ($data['password'] != $data['confirm_password']) {
        $data['confirm_password_err'] = 'password fields do not match';
      }
